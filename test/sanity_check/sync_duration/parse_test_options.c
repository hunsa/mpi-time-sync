...
void init_parameters(clock_drift_opts_t* opts_p, char* name) {
    opts_p->n_rep = 0;
    opts_p->steps = 0;
...
int parse_test_options(clock_drift_opts_t* opts_p, int argc, char **argv) {
    int c;
    ...
    if (opts_p->n_rep <= 0) {
        fprintf(stderr, "Invalid number of repetitions (should be positive): %ld\n", opts_p->n_rep);
        exit(1);
    }
